...
async function test() {
  ...
  const markets = [make('at3', 3), make('total', 3, [0.6, 0.4], 'totals', '-more-markets', ' - More Markets'),
    ...
    make('before3', 3 - 1 / 3600000), make('at32', 32, [0.3, 0.7]), make('after32', 32 + 1 / 3600000),
  ...
  assert.deepEqual([...new Set(result.rows.map(r => r.market_id))].sort(), ['at3', 'at32', 'at95', 'below70', 'no80', 'total'].sort());
  assert.equal(result.index.events, 3);
  assert.equal(result.index.filters.min_start_hours, 3);
  assert.equal(result.index.filters.max_start_hours, 32);
  assert.equal(result.index.filters.min_liquidity_usd,100);
  assert.equal(result.index.filters.minimum_probability,0.35);
  assert.equal(result.index.stats.below_min_liquidity,1);
  ...
  assert.equal(result.rows.filter(r => r.market_id === 'no80').length, 1);
  assert.equal(result.rows.filter(r => r.market_id === 'at95').length, 1);
  assert(result.rows.every(r=>r.price>=0.35));
  ...
  console.log('Scanner integration passed: inclusive 3/32h boundaries, 65% threshold, No outcome, complete records, replacement and empty snapshots.');
}
