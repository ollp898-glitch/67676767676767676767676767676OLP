...
function link(dir){if(!fs.existsSync(path.join(dir,'combo-2/index.json')))return;const url='https://raw.githubusercontent.com/ollp898-glitch/67676767676767676767676767676OLP/data/combo-2/index.json';
 ...
 for(const name of ['CHAT-START.md','README.md']){const file=path.join(dir,name);if(!fs.existsSync(file))continue;let text=fs.readFileSync(file,'utf8').replace('External odds are timestamped snapshots; standalone collector status is separate.','External odds are collected once after each scanner update, with a two-second pause between event requests.');if(!text.includes(url))text+=`\n## Combo Layer 2.0 Beta\n\n[Open grouped Combo outcomes and external odds](${url}). External odds are collected once after each scanner update, with a two-second pause between event requests.\n`;fs.writeFileSync(file,text);const entry=manifest?.files.find(f=>f.path===name);if(entry){entry.bytes=Buffer.byteLength(text);entry.sha256=hash(text);}}
