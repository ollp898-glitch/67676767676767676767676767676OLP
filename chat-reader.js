...
function writeChatReader(rows,dir,metadata,verification){
  ...
  const start=heading('BET-X — вход для чтения сканера')+
    `Все сохранённые исходы: **${rows.length}**. Отбор Combo 70%+: **${combo.length}**. Угловые в этом отборе: **${cornerCount}**.\n\n`+
    ...
    `- [Читать Combo 70%+ по спорту и типу рынка](${scopes.combo.url})\n`+
