...
async function test(){
 ...
 const {rows}=flattenMarkets(markets);assert.equal(rows.length,7);assert(!rows.some(r=>r.price<0.2));
 assert.equal(rows.find(r=>r.outcome==='Under').outcome_index,1);assert.equal(rows.find(r=>r.outcome==='Under').token_id,'1-under');
 const ls=ladders(rows),over=ls.find(l=>l.outcome==='Over');assert.deepEqual(over.lines.map(r=>r.line),[1.5,2.5,3.5,4.5]);
 assert.deepEqual(over.lines.map(r=>r.probability_percent),[91.5,77.5,56,34]);
